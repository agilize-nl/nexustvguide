...
export function normalizeEpgTitle(title: string | null | undefined): string {
  if (!title) return "";
  return title
    .normalize("NFD")
...
export class NlzietEpgMatcher {
   ...
   */
  public matchProgramme(
    programme: Programme,
    epgItems: NlzietEpgContent[],
    nlzietChannelId: string
  ): { target: NlzietProgrammeTarget | null; isAmbiguous: boolean; isTimingOrTitleMismatch: boolean } {
    ...
    if (candidates.length === 1) {
      ...
      const target: NlzietProgrammeTarget = {
        kind: "replay",
   ...
   */
  public enrichProgrammes(
    programmes: Programme[],
    epgResponse: NlzietEpgResponse | null,
    channels: Channel[] = [],
    isOutsideEpgWindow = false,
    ...
    const epgByChannel = new Map<string, NlzietEpgContent[]>();
